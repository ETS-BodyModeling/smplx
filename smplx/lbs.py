...
def lbs_sub(
    ...
    mesh = Meshes(verts=[v_posed.squeeze()], faces=[torch.tensor(faces_sub.astype(np.float64),dtype=torch.int32, device=device)])
    subdivided_mesh = subdivider(mesh)
    v_posed =subdivided_mesh.verts_packed().unsqueeze(0)
    faces_posed = subdivided_mesh.faces_packed().unsqueeze(0)
    v_posed = v_posed + D
    # 4. Get the global joint location
    J_transformed, A = batch_rigid_transform(rot_mats, J, parents, dtype=dtype)
